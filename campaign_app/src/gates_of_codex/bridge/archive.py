...
class CampaignSaveArchive:
    ...
    def read(self, path: str | Path) -> CampaignSaveContents:
        ...
        if not source.is_file():
            raise FileNotFoundError(f"Campaign save does not exist: {source}")
        try:
            with zipfile.ZipFile(source, "r") as archive:
                status = archive.read(self.STATUS_NAME).decode("utf-8-sig", errors="replace")
                campaign_scn = archive.read(self.SCN_NAME).decode(
                    "utf-8-sig", errors="replace"
                )
        except KeyError as exc:
            raise ValueError(f"Campaign save is missing required entry: {exc}") from exc
        except zipfile.BadZipFile as exc:
            raise ValueError(f"Campaign save is not a valid archive: {source}") from exc
