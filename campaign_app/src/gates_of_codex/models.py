...
class BattalionRosterEntry:
    ...
    def validate(self) -> None:
        ...
        if self.quantity < 0:
            raise ValueError(f"Roster quantity cannot be negative: {self.quantity}")
    ...
    def validate(self) -> None:
        ...
        for key, province in self.provinces.items():
            if key != province.province_id:
                raise ValueError(f"Province key mismatch: {key} != {province.province_id}")
            province.validate()
            for neighbor_id in province.neighbors:
                if neighbor_id not in self.provinces:
                    raise ValueError(
                        f"Province {province.province_id} references missing neighbor {neighbor_id}"
                    )

        for province in self.provinces.values():
            for neighbor_id in province.neighbors:
                neighbor = self.provinces[neighbor_id]
                if province.province_id not in neighbor.neighbors:
                    raise ValueError(
                        f"Adjacency must be reciprocal: {province.province_id} -> {neighbor_id}"
                    )

        occupied: dict[str, str] = {}
        for battalion_id, battalion in self.battalions.items():
            if battalion_id != battalion.battalion_id:
                raise ValueError(f"Battalion key mismatch: {battalion_id}")
            battalion.validate()
            if battalion.province_id not in self.provinces:
                raise ValueError(
                    f"Battalion {battalion_id} references missing province {battalion.province_id}"
                )
            previous = occupied.setdefault(battalion.province_id, battalion_id)
            if previous != battalion_id:
                raise ValueError(
                    f"Province {battalion.province_id} contains multiple primary battalions: "
                    f"{previous}, {battalion_id}"
                )
