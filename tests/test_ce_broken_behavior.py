...
class CeBrokenBehaviorTests(unittest.TestCase):
    ...
    def test_control_handoff_resumes_issued_escape(self) -> None:
        beh = BEH.read_text(encoding="utf-8")
        human = HUMAN.read_text(encoding="utf-8")
        self.assertIn("broken/control_hold", beh)
        self.assertIn("broken/resume", beh)
        hold = beh.split("broken/control_hold", 1)[1].split("broken/resume", 1)[0]
        resume = beh.split("broken/resume", 1)[1].split("broken/arrive", 1)[0]
        escape = beh.split("broken/escape", 1)[1].split("broken/control_hold", 1)[0]
        self.assertIn("{state user_control}", hold.split("{actions", 1)[0])
        self.assertIn("{tag aio_morale_retreat_issued}", hold)
        self.assertIn("{tag_add aio_morale_needs_resume}", hold)
        self.assertNotIn("{action move}", hold)
        self.assertIn("{state user_control}", resume.split("{actions", 1)[0].split("{exclude", 1)[1])
        self.assertIn("{tag aio_morale_needs_resume}", resume.split("{actions", 1)[0])
        self.assertIn("{tag_add aio_morale_retreat_wave}", resume)
        self.assertIn("{tag_remove aio_morale_needs_resume}", resume)
        self.assertIn('{name "conquest_enhanced_mechanics/broken/escape"}', resume)
        self.assertNotIn("{action move}", resume)
        self.assertIn("{time 0.5}", resume)
        self.assertNotIn("broken/control_hold", escape)
        rally = human.split('{on "aio_morale_rally"', 1)[1].split('{on "', 1)[0]
        die = human.split('{on "die"', 1)[1].split("{on ", 1)[0]
        self.assertIn('{tags remove "aio_morale_needs_resume"}', rally)
        self.assertIn('{tags remove "aio_morale_needs_resume"}', die)
        cleanup = beh.split("broken/cleanup_dead", 1)[1]
        self.assertIn("tag_remove aio_morale_needs_resume", cleanup)
