...
class CeBrokenBehaviorTests(unittest.TestCase):
    ...
    def test_surrender_presentation_and_cleanup(self) -> None:
        ...
        self.assertNotIn("{volume in_hands}", beh)
        ...
        self.assertLess(present.find("{tag_remove enemy}"), present.find("{action drop}"))
        self.assertLess(present.find("{action drop}"), present.find("{collage stand_giveup_1}"))
    ...
    def test_surrender_evacuates_to_captor_entry(self) -> None:
        ...
        self.assertRegex(ctf, r"\{scoreFinal\s+900\}")
        self.assertNotRegex(ctf, r"\{scoreFinal\s+9000\}")
        self.assertIn("points_table_player=0/0.000,0.333/0.416,0.5/0.555,0.667/0.667,1.00/0.833", ctf)
        self.assertIn("points_table_ai=0/0.000,0.333/0.139,0.50/0.167,0.667/0.208,1.00/0.277", ctf)
        self.assertIn("kill_score_multiplier=0", ctf)
