...
class OldBoyProductionSurrenderTests(unittest.TestCase):
    def test_production_present_is_old_boy_five_step(self) -> None:
        ...
        self.assertIn('{"inventory"', present)
        self.assertIn("{action drop}", present)
        self.assertIn("{type using}", present)
        self.assertIn('{item "weapon"}', present)
        self.assertIn('{item "rocketlauncher"}', present)
        self.assertEqual(present.count("{action drop}"), 3)
        self.assertEqual(present.count('{item "weapon"}'), 2)
        self.assertEqual(present.count('{item "rocketlauncher"}'), 1)
        self.assertNotIn("{type existance}", present)
        self.assertNotIn('{item "at_rifle"}', present)
        self.assertLess(present.find('{item "weapon"}'), present.find('{item "rocketlauncher"}'))
        self.assertLess(present.find('{item "rocketlauncher"}'), present.find("{collage stand_giveup_1}"))
    ...
    def test_parked_mirror_and_diagnostics_are_gone(self) -> None:
        ...
        self.assertIn("{Impregnability harmless}", transfer)
        self.assertIn("{impregnability harmless}", transfer)
