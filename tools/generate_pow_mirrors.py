...
def strip_inventory(text: str) -> str:
    block = find_block(text, "inventory")
    if block is None:
        return text
    start, end = block
    nl = detect_nl(text)
    line_start = text.rfind(nl, 0, start)
    if line_start == -1:
        cut = 0
    else:
        cut = line_start
...
def transform(rel: str, text: str) -> str:
    ...
    if re.search(r"\{inventory\b", text):
        raise ValueError("inventory_remained")
    if brace_delta(text) != 0:
