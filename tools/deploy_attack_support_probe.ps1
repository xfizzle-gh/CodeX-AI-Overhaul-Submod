...
foreach ($source in @($gameSetSource, $botMainSource, $supportSource, $varsSource, $retaskSource, $tplSource, $conquestSource, $utilitySource)) {
...
if (Select-String -Quiet -LiteralPath $retaskSource -SimpleMatch '{tag attack_support_probe}') {
    throw "Source retask probe still gates on attack_support_probe. That tag is a best-effort marker only; key on attack_support_src, which is proven queryable on these units"
}
if (Select-String -Quiet -LiteralPath $retaskSource -SimpleMatch '{tag_remove attack_support_src}') {
    throw "Source retask probe removes attack_support_src, but the entire downstream chain selects on it"
}
...
foreach ($marker in @(
    ...
    '{Tags "attack_support_tpl" "attack_support_w1" "hidden" 0xaf25}',
    '{Tags "attack_support_tpl" "attack_support_w2" "hidden" 0xaf34}',
    '{Tags "attack_support_tpl" "attack_support_w3" "hidden" 0xaf42}',
...
Write-Host "Deploying command-gated attack support wave engine (W1 30s / W2 90s / W3 150s)"
...
$probeInclude = '(include "../attack_support_probe.inc")'
...
foreach ($mapFile in $mapFiles) {
    # Legacy cleanup for maps written by a pre-rename deploy. The old attack_mate_*
    # files no longer exist, so an inherited include line is a dangling reference
    # that the engine cannot resolve. Strip the old include lines outright; the
    # renamed includes are then (re)added by the idempotent blocks below.
    ...
    if ($text -ne $legacyBefore) {
        ...
        Write-Host "LEGACY-STRIPPED attack_mate includes from $mapFile"
    }
...
foreach ($marker in @('{"attack_support_probe_started"}', '{"attack_support_wave_cmd"}', '{"attack_support_use_mi"}')) {
    if (-not (Select-String -Quiet -LiteralPath $vars -SimpleMatch $marker)) {
        throw "Workshop dcg_vars.inc is missing the attack support probe state: $marker"
    }
}
...
if (-not (Select-String -Quiet -LiteralPath $retask -SimpleMatch '{var "attack_support_wave_cmd$"}')) {
    throw "Workshop retask probe is missing the wave command gate - without it the waves all fire at once on entity presence"
}
foreach ($wave in @('{"attack_support/wave1"', '{"attack_support/wave2"', '{"attack_support/wave3"')) {
...
foreach ($n in 1..3) {
    if (-not (Select-String -Quiet -LiteralPath $retask -SimpleMatch ('{selector {tag attack_support_w' + $n + '}}'))) {
...
if (-not (Select-String -Quiet -LiteralPath $retask -SimpleMatch '{tag_add attack_support_flag1}')) {
    throw "Workshop retask probe is not claiming a real flag point"
}
if (-not (Select-String -Quiet -LiteralPath $retask -SimpleMatch '{group {select {tag {tag attack_support_deploy}}}}')) {
    throw "Workshop retask probe is not using the proven bare select form"
}
if (Select-String -Quiet -LiteralPath $retask -SimpleMatch '{tag attack_support_probe}') {
    throw "Workshop retask probe still gates on the unproven attack_support_probe marker"
}
...
Select-String -LiteralPath $vars -Pattern "attack_support_probe_|attack_support_wave_cmd|attack_support_use_mi"
Select-String -LiteralPath $retask -Pattern 'user_is_defender|attack_support_wave_cmd|attack_support/wave|ATTACK SUPPORT WAVE'
