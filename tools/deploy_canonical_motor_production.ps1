...
$entryHotfix = Join-Path $RepoRoot "tools\apply_motor_entry_waypoint_hotfix.py"
...
if (-not (Test-Path -LiteralPath $entryHotfix)) {
    throw "Missing motor entry-waypoint hotfix: $entryHotfix"
}
...
# and package tags. The production overlay intentionally generates common
...
# The base map deployer validates numbered pads for physical placement. Bare
# attack_support_entry_a/b aliases are movement routes only. Apply this last so
# the production overlay cannot replace the numbered placement targets again.
& python $entryHotfix --root $WorkshopRoot
if ($LASTEXITCODE -ne 0) {
    throw "Motor numbered-entry placement hotfix failed with exit code $LASTEXITCODE."
}
& python $entryHotfix --root $WorkshopRoot --check
if ($LASTEXITCODE -ne 0) {
    throw "Motor numbered-entry placement did not validate."
}
...
@(
    ...
    "placement=hull_only_at_numbered_base_entry_links_carry_occupants"
    "placement_waypoints=attack_support_entry_a1|attack_support_entry_b1"
    "pre_drive_actor_state=hull_only"
    "cab_crew=driver_and_commander_remain_linked"
    "passenger_ai=post_emit_only"
    "withdrawal=return_to_bare_base_entry_alias"
...
Write-Host "  Placement:   hull-only on numbered a1/b1 pads; bare aliases reserved for return movement"
Write-Host "  Seats:       linked cab and cargo preserved"
