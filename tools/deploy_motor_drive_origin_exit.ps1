...
$correction = Join-Path $RepoRoot "tools\apply_motor_drive_origin_exit.py"
...
if (-not (Test-Path -LiteralPath $correction)) {
    throw "Missing motor drive/exit correction: $correction"
}

# Rebuild the exact tested Stage-2 package first. Only its branch assertion is
# changed in this ephemeral wrapper so it can run from the stacked correction branch.
